...
export class RevitError extends Error {
  ...
  constructor(
    message: string,
    opts: { error_code?: string; hint?: string; recovery?: string[]; details?: Record<string, unknown> } = {}
  ) {
    super(message);
 ...
 */
export const ERROR_HINTS: Record<string, { description: string; primary_action: string; diagnostic_tool: string }> = {
  connection: {
