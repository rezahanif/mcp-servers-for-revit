...
export function registerListRevitApiCategoriesTool(server: McpServer) {
  server.tool(
    "list_revit_api_categories",
    "Browse Revit API namespaces by category. Returns all 30 namespaces grouped by domain (core, architecture, structural, mep, etc.).",
    {
      category: z
        .string()
        .optional()
      ...
      try {
        ...
        if (args.category) {
          namespaces = namespaces.filter(
            (ns: any) => ns.relevance === args.category
          );
        }
