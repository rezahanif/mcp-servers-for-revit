...
export function registerListRevitTemplatesTool(server: McpServer) {
  server.tool(
    "list_revit_templates",
    "Browse available Revit workflow templates — tested step-by-step guides for common BIM operations.",
    {
      stage: z
        .string()
        .optional()
    ...
    async (args, extra) => {
      try {
        ...
        if (args.stage) {
          templates = templates.filter((t: any) => t.stage === args.stage);
        }
...
export function registerLoadRevitTemplateTool(server: McpServer) {
  server.tool(
    "load_revit_template",
    "Load a full Revit workflow template — returns step-by-step instructions with tool calls and parameters.",
    {
      id: z
        .string()
    ...
    async (args, extra) => {
      try {
        const data = loadTemplates();
        const template = data.templates.find((t: any) => t.id === args.id);

        if (!template) {
          const available = data.templates.map((t: any) => t.id);
          return {
            content: [
              {
                type: "text",
