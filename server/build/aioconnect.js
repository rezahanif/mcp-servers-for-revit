...
export async function ensureLicensed() {
    if (process.env.AICONNECT_ENABLE !== "1")
        return null;
    const mod = await sdk();
    const validator = new mod.LicenseValidator(process.env.JWT_SECRET ?? "dev-secret-change-me");
...
export async function envelope(text) {
