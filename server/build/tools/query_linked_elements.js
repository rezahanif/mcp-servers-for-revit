...
export function registerQueryLinkedElementsTool(server) {
    server.tool("query_linked_elements", "Query elements inside a specific linked model, with category filtering, parameter filters, and custom return fields. Coordinates are automatically transformed into main-model space using the link's transform, so results align with the host document. Get linkInstanceId from get_linked_models first.", {
