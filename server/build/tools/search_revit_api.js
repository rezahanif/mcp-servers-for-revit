...
export function registerSearchRevitApiTool(server) {
    server.tool("search_revit_api", "Search the Revit API by keyword — finds matching namespaces, classes, and descriptions. Use this to discover what API classes exist before writing send_code_to_revit code.", {
        query: z
            .string()
        ...
        try {
            ...
            for (const ns of index.namespaces) {
                const matchedClasses = ns.classes.filter((c) => c.toLowerCase().includes(query));
