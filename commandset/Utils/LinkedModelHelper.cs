    ...
    public class LinkedModelHelper
    {
        ...
        public object GetLinkedModels()
        {
            ...
            foreach (var link in linkInstances)
            {
                ...
                results.Add(new
                {
                    LinkInstanceId = link.Id.GetIntValue(),
                    LinkTypeName = linkTypeName,
                    FileName = linkDoc?.Title ?? "(unloaded)",
                    FilePath = linkDoc?.PathName ?? "(no path)",
        ...
        {
            Document doc = _uiApp.ActiveUIDocument.Document;
            long linkInstanceId = parameters["linkInstanceId"]?.ToObject<long>() ?? 0;
            string categoryName = parameters["category"]?.ToObject<string>();
            JArray filters = parameters["filters"] as JArray;
            JArray returnFields = parameters["returnFields"] as JArray;
            int maxCount = parameters["maxCount"]?.ToObject<int>() ?? 500;
...
#else
            var linkInstance = doc.GetElement(new ElementId((int)linkInstanceId)) as RevitLinkInstance;
#endif
            if (linkInstance == null)
                throw new InvalidOperationException($"Linked model instance not found with ID: {linkInstanceId}");

            var linkDoc = linkInstance.GetLinkDocument();
            if (linkDoc == null)
                throw new InvalidOperationException("Linked model is not loaded");
        ...
        public object GetElementGeometry(JObject parameters)
        {
            ...
            long? linkInstanceId = parameters["linkInstanceId"]?.ToObject<long>();
            string geometryType = parameters["geometryType"]?.ToObject<string>() ?? "centerline";
            bool applyTransform = parameters["applyTransform"]?.ToObject<bool>() ?? true;
...
#else
                var linkInstance = doc.GetElement(new ElementId((int)linkInstanceId.Value)) as RevitLinkInstance;
#endif
                if (linkInstance == null)
                    throw new InvalidOperationException($"Linked model instance not found with ID: {linkInstanceId.Value}");
...
#else
            Element element = targetDoc.GetElement(new ElementId((int)elementId));
#endif
            if (element == null)
                throw new InvalidOperationException($"Element not found with ID: {elementId}");
            ...
            switch (geometryType.ToLowerInvariant())
            {
                ...
                case "all":
                    ...
                    break;
                default:
                    throw new ArgumentException($"Unsupported geometryType: {geometryType}. Supported: centerline, boundingbox, solid, all");
            }
        ...
        public static BuiltInCategory ResolveBuiltInCategory(string categoryName)
        {
            ...
            if (lower == "pipes" || lower == "管" || lower == "管線") return BuiltInCategory.OST_PipeCurves;
            if (lower == "ducts" || lower == "風管") return BuiltInCategory.OST_DuctCurves;
            if (lower == "cabletrays" || lower == "cable trays" || lower == "電纜架") return BuiltInCategory.OST_CableTray;
            if (lower == "conduits" || lower == "導管" || lower == "電管") return BuiltInCategory.OST_Conduit;
            if (lower == "pipefittings" || lower == "pipe fittings" || lower == "管件") return BuiltInCategory.OST_PipeFitting;
            if (lower == "ductfittings" || lower == "duct fittings" || lower == "風管管件") return BuiltInCategory.OST_DuctFitting;
            if (lower == "pipeaccessories" || lower == "pipe accessories" || lower == "管配件") return BuiltInCategory.OST_PipeAccessory;
            if (lower == "walls" || lower == "牆" || lower == "牆壁") return BuiltInCategory.OST_Walls;
            if (lower == "floors" || lower == "樓板" || lower == "板") return BuiltInCategory.OST_Floors;
            if (lower == "structuralframing" || lower == "structural framing" || lower == "結構構架" || lower == "樑") return BuiltInCategory.OST_StructuralFraming;
            if (lower == "structuralcolumns" || lower == "structural columns" || lower == "結構柱") return BuiltInCategory.OST_StructuralColumns;
            if (lower == "columns" || lower == "建築柱") return BuiltInCategory.OST_Columns;
            if (lower == "ceilings" || lower == "天花板") return BuiltInCategory.OST_Ceilings;
            if (lower == "doors" || lower == "門") return BuiltInCategory.OST_Doors;
            if (lower == "windows" || lower == "窗") return BuiltInCategory.OST_Windows;
            if (lower == "rooms" || lower == "房間") return BuiltInCategory.OST_Rooms;
            if (lower == "genericmodels" || lower == "generic models" || lower == "一般模型") return BuiltInCategory.OST_GenericModel;

            if (Enum.TryParse<BuiltInCategory>(categoryName, true, out var parsed)) return parsed;
            if (!categoryName.StartsWith("OST_") && Enum.TryParse<BuiltInCategory>("OST_" + categoryName, true, out parsed)) return parsed;

            throw new ArgumentException($"Cannot resolve BuiltInCategory for '{categoryName}'");
        }
        ...
        private object ExtractCenterline(Element element, Transform transform)
        {
            ...
            return new { HasCenterline = false, Message = "Element has no centerline" };
        }
        ...
        private object ExtractBoundingBox(Element element, Transform transform)
        {
            ...
            if (bbox == null)
                return new { HasBoundingBox = false, Message = "Could not get BoundingBox" };
        ...
        private object ExtractSolidInfo(Element element, Transform transform)
        {
            try
            {
                ...
                if (geoElement == null)
                    return new { HasSolid = false, Message = "No geometry" };
            ...
            {
                return new { HasSolid = false, Message = $"Geometry extraction failed: {ex.Message}" };
            }
        }
    }
}
